...
parser.add_argument("--weights_path", type=str, default="checkpoints/yolov3-coco-trained.weights", help="path to weights file")
...
sub_batch_size = 3
subdivisions = int(opt.batch_size / sub_batch_size)
print("start traing")
...
for epoch in range(opt.epochs):
    ...
    for batch_i, (_, imgs, targets) in enumerate(dataloader):
        ...
        optimizer.zero_grad()
        for sub_batch_i in range(subdivisions):
            sub_img = imgs[sub_batch_i * sub_batch_size : (sub_batch_i + 1) * sub_batch_size]
            sub_target = targets[sub_batch_i * sub_batch_size : (sub_batch_i + 1) * sub_batch_size]
            loss = model(sub_img, sub_target)
            loss.backward()
           
        optimizer.step()
        #a batch_size step onec
